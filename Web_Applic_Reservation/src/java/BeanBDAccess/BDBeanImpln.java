...
public class BDBeanImpln implements BDBean, Serializable {
    ...
    {
        List<Emplacement> returnValue = new ArrayList<>();
        
        try 
        {
            Connection connection = getConnection();
            Statement statement = connection.createStatement();
            ResultSet resultSet = statement.executeQuery("SELECT * FROM emplacements");
          
            while(resultSet.next()) {
                returnValue.add(new Emplacement(resultSet));
            } 
        }
        catch (SQLException ex) {
            Logger.getLogger(BDBeanImpln.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public boolean addReservation(Reservation r)
    {
        ...
        {
            Connection connection = getConnection();
            PreparedStatement pstmt = connection.prepareStatement("INSERT INTO reservations VALUES(?,?,?,?,?)");
            pstmt.setString(1, reservation.getIdReservation());
            pstmt.setString(2, reservation.getIdContainer());
            pstmt.setString(3, reservation.getIdSociete());
            pstmt.setString(4, reservation.getIdTransporteur());
            pstmt.setString(5, reservation.getDateReservation());
            ret = pstmt.executeUpdate();
        }
        catch (SQLException ex) {
            Logger.getLogger(BDBeanImpln.class.getName()).log(Level.SEVERE, null, ex);
    ...
    {    
        ...
        {
            Connection connection = getConnection();
            PreparedStatement pstmt = connection.prepareStatement("UPDATE emplacements SET idContainer = ? WHERE idEmplacement = ?");
            pstmt.setString(1, idContainer);
            pstmt.setInt(2, idEmplacement);
            ret = pstmt.executeUpdate();
        }
        catch (SQLException ex) {
            Logger.getLogger(BDBeanImpln.class.getName()).log(Level.SEVERE, null, ex);
